...
class MipNerfModel(nn.Module):
  ...
  def __call__(
    ...
    for i_level in range(self.num_levels):
      ...
      rgb_flip, pi_flip, gamma_flip, weights_flip, feature_dist_flip, flip_mask = None, None, None, None, None, None
      ...
      if self.use_viewdirs:
        (rgb, density, normals, gamma, feature_dist) = mlp(rng, samples, rays.viewdirs)
      else:
      ...
      if train and i_level == 0:
        ...
        flip_directions = 2.0 * jnp.sum(
            normals_to_use * rays.directions, axis=-1, keepdims=True) * normals_to_use - rays.directions
        flip_viewdirs = flip_directions / jnp.linalg.norm(flip_directions, axis=-1, keepdims=True)

        # find the hitting point, i.e., estimated object surface
        t_mids = 0.5 * (t_vals[Ellipsis, :-1] + t_vals[Ellipsis, 1:])
        hitting_point = t_mids[jnp.arange(weights.shape[0]), jnp.argmax(weights, axis=-1)][Ellipsis, None]

        # set the origin of the flipped reflection ray
        flip_origins = (rays.origins + hitting_point * rays.directions) - (hitting_point * flip_directions)
        ...
        (rgb_flip, density_flip, _, gamma_flip, feature_dist_flip) = mlp(rng, samples_flip, flip_viewdirs)
        weights_flip, _, _, _ = mip.compute_alpha_weights(density_flip, t_vals_flip, flip_directions)
        pi_flip = weights_flip / (jnp.sum(weights_flip, axis=1, keepdims=True) + 1e-12)
