...
def train_step(
  ...
  def compute_nll(mu, pi, gamma, gt, mask, dist='laplace'):
      ...
      md_lh = jnp.nan_to_num(jnp.sum(pi * multi_lh, axis=1))
  ...
  def loss_fn(variables):
    ...
    for i, rendering in enumerate(renderings):      
      ...
      if i == 0:
        if config.nll_loss:
          ...
          flip_nll_loss = compute_nll(rendering['mu_flip'], rendering['pi_flip'], rendering['gamma_flip'], batch['rgb'][:, None, :], rendering['flip_mask'], dist='laplace')
          losses.append(flip_nll_loss.sum() / (rendering['flip_mask'].sum() + eps))
        else:
          losses.append(0.0)
          losses.append(0.0)

        if config.ue_loss:
          rho = jax.lax.stop_gradient(rendering['gamma'].sum(axis=1).mean(axis=-1, keepdims=True))
          ue_loss = jnp.mean(jnp.log(eps + 1.0 + config.eta * rho * rendering['weights']), axis=-1)
          losses.append(ue_loss.sum() / lossmult.sum())
          # for flipped reflection rays
          flip_rho = jax.lax.stop_gradient(rendering['gamma_flip'].sum(axis=1).mean(axis=-1, keepdims=True))
          flip_ue_loss = jnp.mean(jnp.log(eps + 1.0 + config.eta * flip_rho * rendering['weights_flip']), axis=-1)
          losses.append((flip_ue_loss * rendering['flip_mask'][:, 0]).sum() / (rendering['flip_mask'].sum() + eps))
        else:
          losses.append(0.0)
          losses.append(0.0)

        if config.bfc_loss:
          m = jnp.nan_to_num((rendering['feature_dist'] + rendering['feature_dist_flip']) * 0.5) + eps
          a = jnp.sum(rendering['feature_dist'] * jnp.log(rendering['feature_dist'] / m + eps), axis=-1)
          b = jnp.sum(rendering['feature_dist_flip'] * jnp.log(rendering['feature_dist_flip'] / m + eps), axis=-1)
          bfc_loss = jnp.mean((a + b) * 0.5, axis=-1)
          losses.append((bfc_loss * rendering['flip_mask'][:, 0]).sum() / (rendering['flip_mask'].sum() + eps))
        else:
          losses.append(0.0)

        if config.ori_loss:
          ori_loss = (rendering['weights'] * (jnp.maximum(jnp.nan_to_num((rendering['normals'] * rendering['viewdirs'][:, None, :])).sum(axis=-1), 0.0) ** 2 + eps)).sum(axis=-1)
          losses.append(ori_loss.sum() / lossmult.sum())
        else:
          losses.append(0.0)
    ...
    loss = (losses[-1] +
            ...
            flip_nll_loss_weight * losses[2] +
            ue_loss_weight * losses[3] +
            config.flip_ue_loss_weight * losses[4] +
            config.bfc_loss_weight * losses[5] +
            config.ori_loss_weight * losses[6] +
...
def main(unused_argv):
  ...
  for step, batch in zip(range(init_step, config.max_steps + 1), pdataset):
    ...
    if config.flip_nll_loss_decay:
      flip_nll_loss_weight = math.compute_tvnorm_weight(
          step, config.flip_nll_loss_maxstep,
          config.flip_nll_loss_weight_start,
          config.flip_nll_loss_weight_end)
    else:
      flip_nll_loss_weight = config.flip_nll_loss_mult

    if config.ue_loss_decay:
      ue_loss_weight = math.compute_tvnorm_weight(
          step, config.ue_loss_maxstep,
          config.ue_loss_weight_start,
          config.ue_loss_weight_end)
    else:
      ue_loss_weight = config.ue_loss_mult
